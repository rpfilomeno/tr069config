...
class AutoConfigCommand extends Command
{
    ...
    function options($opts)
    {
        ...
        $opts->add('s|secure', 'force https connection.');
        $opts->add('h|hash-password', 'provide password as a hash rather than plain-text.');
        $opts->add('d|download', 'attempt to download the xml configuration file is no matching store one is found.');
    }
    ...
    function execute($eSpaceUsername, $eSpacePassword, $csvConfigFilename, $txtConfigFilename)
    {
        ...
        foreach ($txtConfigs as $deviceIpDetailsLine) {
            try {
                ...
                foreach ($altXmlConfigFilenames as $altXmlConfigFilename) {
                    ...
                    $this->logger->debug('Looking for stored xml configuration file  ' . $testAltXmlConfigFilename . ' for ' . $deviceIp);
                    if (file_exists($testAltXmlConfigFilename)) {
                        ...
                        $this->logger->debug('Configuration file  ' . $testAltXmlConfigFilename . ' for ' . $deviceIp . ' not found.');
                    }
                }

                if(!$xmlConfigFound && $this->options->has('download')) { //no match, try to download

                } elseif(!$xmlConfigFound) { //no match, give up!
                    $this->logger->error('No matched configuration file for ' . $deviceIp .' (try --download)');
                    continue;
                }

                $this->logger->debug('Using best matched configuration file  ' . $xmlConfigFilename . ' for ' . $deviceIp);
